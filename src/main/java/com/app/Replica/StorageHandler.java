        ...
        try {
            ...
            while ((request = input.readLine()) != null) {
                ...
                if ("ACQUIRE_LOCK".equals(method) && parts.length == 4) {
                    logger.info("Acquiring write lock for key: " + parts[3]);
                    String key = parts[3];
                    ...
                    logger.info("Releasing lock for key " + parts[3]);
                    String key = parts[1];
                    InetSocketAddress clientAddress = new InetSocketAddress(
                            clientSocket.getInetAddress().getHostAddress(), clientSocket.getPort());
                    ...
                    logger.info("Trying to update stale response for key: " + parts[3]);
                    String key = parts[3];
                    String value = parts[4];
